...
public class SetEventStatusHandler : ICommandHandler<SetEventStatusCommand>
{
    private readonly IEventRepository repository;
    private readonly IUnitOfWork uow;

    public SetEventStatusHandler(IEventRepository repository, IUnitOfWork uow)
    ...
    public async Task<Result<None>> HandleAsync(SetEventStatusCommand? command)
    {
        ...
        Result<Event> result = evt.SetEventStatus(command.Status);
