...
public class AcceptInvitationHandler : ICommandHandler<AcceptInvitationCommand>
{
    ...
    public AcceptInvitationHandler(IGuestRepository guestRepository, IEventRepository eventRepository, IUnitOfWork uow)
    {
    ...
    public async Task<Result<None>> HandleAsync(AcceptInvitationCommand command)
    {
        ...
        var result = guest.AcceptInvitation(new Invitation(_event, guest));
        if (result.IsFailure())
        {
